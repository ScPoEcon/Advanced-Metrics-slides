...
corrplot::corrplot(ca,addgrid.col = FALSE,tl.pos = "n",na.label = " ")
```
]
...
ames_cdf = coef(ames_lasso, s = "lambda.1se") %>%
    as.matrix() %>%
    data.frame()
ames_cdf$variable = rownames(ames_cdf)
rownames(ames_cdf) <- NULL
ames_cdf$variable = reorder(ames_cdf$variable,ames_cdf$s1)

ames_cdf[1:50,] %>%
    filter(variable != "(Intercept)") %>%
    ggplot(aes(s1, reorder(variable, s1), color = s1 > 0)) +
